...
static ConVar analyzer_strafetrainer_type(
    "mom_hud_strafetrainer_type", "1", FCVAR_CLIENTDLL | FCVAR_CLIENTCMD_CAN_EXECUTE | FCVAR_ARCHIVE,
    "1: Filled graph\n 2: Line Graph\n 3: Horizontal Bar\n 4: Vertical Bar\n", true, 1, true, 4);

static MAKE_TOGGLE_CONVAR(analyzer_strafetrainer_colorize, "1", FCVAR_CLIENTCMD_CAN_EXECUTE | FCVAR_ARCHIVE,
                          "Toggles strafetrainer data colorization type based on sync and speedloss. 0 to disable and set to default static color\n");
...
{
    Color colorPerf = {255, 255, 255, 120}, 
        colorSynced = {61, 79, 115, 255}, 
        colorNotSynced = {216, 162, 55, 255},
        colorLeft = {61, 79, 115, 255}, 
        colorRight = {216, 162, 55, 255}, 
        colorCS = {0, 0, 0, 255},
...
{
    if (!(History::strafes.Size() > 1)) return;
    if (!(mom_hud_analyzer_strafetrainer.GetBool())) return;
        ...
        for (int i = 0; i < HISTORY_SIZE; i++)
        {
            ...
            if (History::strafes[i].perfDeltaYaw < MAX_ANGLE)
            {
                ...
                surface()->DrawFilledRect(xOffset + (DATA_WIDTH * i),
                                          yOffset - (DATA_HEIGHT * History::strafes[i].perfDeltaYaw), 
                                          DATA_WIDTH, (DATA_HEIGHT * History::strafes[i].perfDeltaYaw));
            }
            ...
            if (History::strafes[i].deltaYaw < MAX_ANGLE)
            {
                ...
                surface()->DrawFilledRect(xOffset + (DATA_WIDTH * i),
                                          yOffset - (DATA_HEIGHT * History::strafes[i].deltaYaw), 
                                          DATA_WIDTH, (DATA_HEIGHT * History::strafes[i].deltaYaw));
            }
        }
    }
    else if (analyzer_strafetrainer_type.GetInt() == 1) // Line Graph
    {
        for (int i = 0; i < HISTORY_SIZE - 1; i++)
        {
            ...
            if (History::strafes[i].perfDeltaYaw < MAX_ANGLE && History::strafes[i + 1].perfDeltaYaw < MAX_ANGLE)
            {
                ...
                for (int j = 0; j < 2; j++)
                {
                    surface()->DrawLine(xOffset + (DATA_WIDTH * i),
                                        yOffset - (DATA_HEIGHT * History::strafes[i].perfDeltaYaw) + j,
                                        xOffset + (DATA_WIDTH * i),
                                        yOffset - (DATA_HEIGHT * History::strafes[i + 1].perfDeltaYaw) + j);
                }
            }
